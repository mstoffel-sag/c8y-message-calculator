...
src/ui/wizard/    the seven steps and the hand-off table
...
Steps 2, 3 and 4 all edit the same machine types from different angles, so past
two types every one of them was a page nobody reads. Each block is a `<details>`
...
- **The parts come before the measurement count.** "10 datapoints in 3
  measurement types" would be false -- the event, alarm, fact and command are not
  in a measurement at all.

/**
 * Every figure in the app, formatted.
 *
 * In lib/ rather than in an app because both apps show the same numbers and
 * German has opinions about all of them: 1,000.5 and 1.000,5 are the same
 * number and different strings, and "46.0 M" is "46,0 Mio.". Getting that right
 * once is worth more than either app's convenience.
 *
 * Copying to the clipboard is *not* here -- it needs a DOM, which lib/ may not
 * have. Each app keeps its own.
 */
import { splitDuration, type DurationUnit } from '../engine/duration.js';
import {
  DEFAULT_LOCALE,
  LOCALES,
  translate,

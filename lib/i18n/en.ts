...
export const en = {
  ...
  'series.teach.body': 'A measurement carries **one timestamp** and any number of series underneath it. One `POST` is **one message** whether it carries one series or forty. So the question that decides your volume is not how much data you send — it is **how many requests you spread it across**.\n\nThat makes the sampling interval the natural grouping: everything sampled on the same tick shares a timestamp, so it can share a measurement. Readings on *different* intervals can never share one, however related they are.\n\n**A flag or a state is a measurement too** — one series, sent when the value moves instead of on a tick. That is the only difference, so it is a column here rather than a section of its own. **Do not put one on a timer:** sent on change, the timestamp is the moment it flipped, which is the information you wanted; sampled every minute, that moment is lost between two ticks and you pay for thousands of identical readings. It also travels alone — its timestamps are its own and can never line up with a shared tick.\n\n**So tell the tool the rhythm and it will design the measurement type.** Every timed series you add drops into the measurement type for its interval automatically, under a suggested name. Both halves of that are yours to change in the *Measurement type* column: rename the fragment your devices actually send, or move one series into a measurement type of its own.',
  'series.choose': 'Choose a series…',
  'series.rhythm.interval': 'On a timer',
  'series.rhythm.onChange': 'When it changes',
  ...
  'series.col.retention': 'Kept for',
  'series.retention.title': 'Days the tenant’s retention rule keeps this measurement type. Leave it empty to use the scenario’s default, set on the Contract step. It changes no message count — only how much is still in the database when the month closes, which is what storage is billed on.',
  'series.retention.inherited': 'scenario default',
  'series.retention.shared': 'as its measurement type',
  ...
  'series.messagesPerMonth': '{count} messages / machine / month',
  'series.solo.onChange': 'one message per change; nothing can share an on-change timestamp',
  ...
  'payload.title.onChange': 'on change only',
  'payload.title.alone': 'its own measurement, every {seconds} s',
  'payload.title.raiseAndClear': '{name} — raise and clear',
  'payload.note.bundle': 'One POST, one timestamp, one message — regardless of how many series it carries. Send exactly this series set every time: a fragment whose shape varies from message to message is what degrades write and query performance.',
  ...
  'payload.note.state': 'Sent only when the value actually changes, so the timestamp is the transition. Do not fold this into an interval bundle: the bundle would have to send a different series set whenever this one moved, and the moment it flipped would be lost between two ticks.',
  ...
  'payload.note.inventory': 'Send this when it changes, not on a timer. The platform does not diff the payload, so a successful PUT that changes nothing still counts.',
  ...
  'engine.naiveBaselineRule': 'Every series in its own measurement, and every state or flag sampled on a timer at the machine type’s fastest interval rather than sent on change. Events, alarms, inventory writes and operations are identical in both models — the whole difference is in the Measurement API.',
  ...
  'storage.retentionDecides': '**Retention decides the size, not the traffic.** Identical traffic held for 90 days occupies three times what it does at 30. This fleet is kept **{kept}**{note} — a rule per measurement type on the Measurements step, over a default and a bytes-per-value figure on the Contract step.',
  'storage.retentionDefault': ' (the starting assumption)',
  'storage.measurementsOnly': 'Measurements only. Events, alarms, inventory writes and operations are stored too, but the source measured datapoints; they are **{share}** of the documents this fleet writes.',
  ...
  'lint.L1.detail': '{names} are sampled on the same tick and mean the same kind of thing, so they can share one measurement and one timestamp. That is one POST instead of {containers}, for identical information.',
  ...
  'lint.L2.title': '“{name}” changes every ~{minutes} min but is sampled every {interval} s',
  'lint.L2.detail': 'That is {ratio}× faster than it changes. Every sample in between stores the value that was already there, and the moment it actually flipped is buried between two ticks. Sent on change it carries the transition timestamp, which is the information.',
  ...
  'lint.L5.detail': 'Inventory is not a time series store. Each PUT bills, overwrites the previous value and leaves nothing to chart. If it changes this often and the history matters, it is a measurement or an event.',
  'lint.L10.title': '“{name}” is re-sent on a timer or at every boot',
  'lint.L10.detail': 'The platform does not diff the payload, so a successful PUT that changes nothing still counts. Re-sending the full managed object on a heartbeat bills every time while storing no new information. Sending it only when it changes removes the whole line. (A rejected write, by contrast, does not count at all — failures are free.)',
  ...
  'explain.rule.body': '**Send the same readings every time.** An envelope whose contents change from one send to the next is what degrades write and query performance later.\n\nWhich is why a flag that changes twice an hour does not belong in a bundle sampled every minute — it would force the envelope to change shape. It gets its own, sent when it actually changes.',

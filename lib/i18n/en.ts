...
export const en = {
  ...
  'discrete.occurrence.teach': 'An event is a **non-numeric** thing that happened, with a timestamp: a door opened, a trip started, a service was performed. Creating one is one message.\n\n**Do not put numbers in events.** A value buried in an event body cannot be aggregated or plotted the way a series can. If it is a number you will want to chart, it is a series.',
  ...
  'series.teach.body': 'A measurement carries **one timestamp** and any number of series under it. Creating one is **one message**, whether it carries one series or forty. So what decides your volume is not how much data you send — it is how many requests you spread it across.\n\nThe sampling interval does the grouping: everything read on the same tick can share a measurement. Readings on *different* intervals never can.\n\n**A flag is a series too**, and the interval you give it is the interval you pay for. Ask how often it actually changes, and read it there.\n\nTell the tool the interval and it designs the measurement type. Rename it, or move a series into one of its own, in the *Measurement type* column.',
  'series.choose': 'Choose a series…',
  'series.heading': 'Series — what this machine measures',
  'series.none': 'None yet. A series is one named value over time: a temperature, a pressure, a motor current, a compressor on/off. Sampled on a timer or sent when it moves — either way it travels in a measurement.',
  ...
  'series.retentionNote': 'A **retention rule belongs to the measurement type**, so it is set once per type here and every series inside one is kept for the same time. Empty means the scenario default of **{days} days** — change that on the Contract step, and override it here for the types your tenant treats differently. Retention moves no counter: it decides how much of what you wrote is still in the database at the end of the month, which is the quantity storage is billed on.',
  ...
  'naive.heading': 'Against the obvious implementation',
  'naive.asDesigned': 'As designed',
  'naive.naive': 'Naive',
  'naive.difference': 'Difference',
  'naive.less': '**{factor}× less volume** — {pct} % — for identical information.',
  'naive.baselineLabel': 'The baseline.',
  'naive.notCost': 'A volume reduction is not automatically a cost reduction: how volume converts to money depends on commercial terms that do not live here. Worth saying anyway — {saving} fewer writes a month buys query latency, headroom and a database that still behaves at scale.',
  'naive.storedValues': '**{count} values stored** in the peak month, as a count — and the input to the storage estimate below.',
  ...
  'storage.odsCell': '**{amount} goes in the ODS cell for period {index}**: what the database held at the end of each of its {months} months, added up, at {bytes} bytes per value — an average of {average} standing in the database per month. That is one figure picked out of a range, and the range is the evidence: {note} Override it in the deployment panel if the tenant has been measured.',
  'storage.bundlingHelps': 'Bundling moves the real figure down inside that range as well as cutting messages: a measurement carrying {count} values pays for its envelope once instead of {count} times, and the 100–400 B figure was measured on values stored one per measurement. The tool does not split the envelope cost from the value cost, because the source measures the two together — so a well-bundled fleet has room below the quoted figure, not above it.',
  'storage.retentionDecides': '**Retention decides the size, not the traffic.** Identical traffic held for 90 days occupies three times what it does at 30. This fleet is kept **{kept}**{note} — a rule per type, asked on the Measurements step and on the events, alarms and commands tables, over a default and a bytes-per-value figure on the Contract step.',
  'storage.retentionDefault': ' (the starting assumption)',
  'storage.documentsToo': 'Events, alarms, operations and every registered device are counted in too — leaving them out would understate the bill, and on a commit-to-consume contract that is the expensive direction. They are **{share}** of what this fleet keeps. The byte figure was measured on datapoints, so that share is the part of the estimate resting on the weaker assumption; a managed object is also the one thing no retention rule removes.',

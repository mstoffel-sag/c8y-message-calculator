...
export const en = {
  ...
  'storage.sub': 'What is on disk on the fullest day — the figure ODS bills',
  'storage.stat.ods': 'Operational data store',
  'storage.stat.ods.sub': 'at {bytes} B / value · {range} across the range',
  'storage.stat.fullest': 'Fullest month',
  'storage.stat.fullest.sub': '{days} days kept',
  ...
  'storage.stat.dataHub': 'DataHub extract',
  'storage.stat.dataHub.sub': '20–25 % of the same data',
  'storage.odsCell': '**{amount} goes in the ODS cell**, at {bytes} bytes per value. That is one figure picked out of a range, and the range is the evidence: {note} Override it in the deployment panel if the tenant has been measured.',
  ...
  'storage.retentionDecides': '**Retention decides the size.** Identical traffic held for 90 days occupies three times what it does at 30. This uses **{days} days**{note} — set that and the bytes per value to the tenant’s own figures on the Contract step.',
  ...
  'item.ods.help': 'Daily maximum storage, in GiB. Filled in from the storage estimate — the values still on disk over the tenant’s retention period, at the assumed bytes per value — and overridable here. The underlying figure is a rule of thumb spanning 100 to 400 bytes and marked “to be verified” at source, so the Storage sheet carries the whole range beside whatever number lands in the cell.',

...
export const de: Record<Key, string> = {
  ...
  'series.teach.body': 'Eine Messung trägt **einen Zeitstempel** und beliebig viele Serien darunter. Ein `POST` ist **eine Nachricht**, ob er eine Serie trägt oder vierzig. Die Frage, die Ihre Menge entscheidet, ist also nicht, wie viele Daten Sie senden — sondern **über wie viele Requests Sie sie verteilen**.\n\nDamit ist das Abtastintervall die natürliche Gruppierung: alles, was auf demselben Takt abgetastet wird, teilt einen Zeitstempel und kann eine Messung teilen. Werte auf *verschiedenen* Intervallen können das nie, so verwandt sie auch sind.\n\n**Ein Schalter oder Zustand ist auch eine Messung** — eine Serie, gesendet wenn sich der Wert bewegt, statt auf einem Takt. Das ist der einzige Unterschied, deshalb ist es hier eine Spalte und kein eigener Abschnitt. **Setzen Sie ihn nicht auf einen Timer:** bei Änderung gesendet ist der Zeitstempel der Moment des Umschaltens, also genau die Information, die Sie wollten; jede Minute abgetastet geht dieser Moment zwischen zwei Takten verloren, und Sie zahlen für Tausende identischer Werte. Er reist auch allein — seine Zeitstempel sind seine eigenen und können sich nie mit einem gemeinsamen Takt decken.\n\n**Sagen Sie dem Werkzeug also den Rhythmus, und es entwirft den Messtyp.** Jede getaktete Serie, die Sie hinzufügen, landet automatisch im Messtyp für ihr Intervall, unter einem vorgeschlagenen Namen. Beide Hälften davon können Sie in der Spalte *Messtyp* ändern: das Fragment umbenennen, das Ihre Geräte wirklich senden, oder eine Serie in einen eigenen Messtyp verschieben.',
  'series.choose': 'Eine Serie wählen…',
  'series.rhythm.interval': 'Auf einem Timer',
  'series.rhythm.onChange': 'Wenn sie sich ändert',
  ...
  'series.messagesPerMonth': '{count} Nachrichten / Maschine / Monat',
  'series.solo.onChange': 'eine Nachricht pro Änderung; einen Änderungszeitstempel kann nichts teilen',
  ...
  'payload.title.onChange': 'nur bei Änderung',
  'payload.title.alone': 'eigene Messung, alle {seconds} s',
  'payload.title.raiseAndClear': '{name} — auslösen und aufheben',
  'payload.note.bundle': 'Ein POST, ein Zeitstempel, eine Nachricht — unabhängig davon, wie viele Serien er trägt. Senden Sie jedes Mal genau diesen Satz an Serien: ein Fragment, dessen Form von Nachricht zu Nachricht variiert, verschlechtert Schreib- und Abfrageleistung.',
  ...
  'payload.note.state': 'Wird nur gesendet, wenn sich der Wert wirklich ändert, der Zeitstempel ist also der Übergang. Falten Sie das nicht in ein Intervallbündel: das Bündel müsste einen anderen Satz an Serien senden, sobald diese sich bewegt, und der Moment des Umschaltens ginge zwischen zwei Takten verloren.',
  ...
  'payload.note.inventory': 'Senden Sie das bei Änderung, nicht auf einem Timer. Die Plattform vergleicht die Payload nicht, ein erfolgreiches PUT, das nichts ändert, zählt also trotzdem.',
  ...
  'engine.naiveBaselineRule': 'Jede Serie in ihrer eigenen Messung, und jeder Zustand oder Schalter auf einem Timer im schnellsten Intervall des Maschinentyps abgetastet, statt bei Änderung gesendet. Ereignisse, Alarme, Inventarschreibvorgänge und Operationen sind in beiden Modellen identisch — der ganze Unterschied liegt in der Measurement-API.',
  ...
  'lint.L1.detail': '{names} werden auf demselben Takt abgetastet und bedeuten dasselbe, sie können also eine Messung und einen Zeitstempel teilen. Das ist ein POST statt {containers}, für dieselbe Information.',
  ...
  'lint.L2.title': '„{name}“ ändert sich etwa alle {minutes} min, wird aber alle {interval} s abgetastet',
  'lint.L2.detail': 'Das ist {ratio}× schneller als die Änderung. Jede Abtastung dazwischen speichert den Wert, der schon da war, und der Moment des Umschaltens liegt begraben zwischen zwei Takten. Bei Änderung gesendet trägt er den Zeitstempel des Übergangs, und der ist die Information.',
  ...
  'lint.L5.detail': 'Inventar ist kein Zeitreihenspeicher. Jedes PUT kostet, überschreibt den vorherigen Wert und lässt nichts zum Darstellen übrig. Wenn sich der Wert so oft ändert und die Historie zählt, ist es eine Messung oder ein Ereignis.',
  'lint.L10.title': '„{name}“ wird auf einem Timer oder bei jedem Start erneut gesendet',
  'lint.L10.detail': 'Die Plattform vergleicht die Payload nicht, ein erfolgreiches PUT, das nichts ändert, zählt also trotzdem. Das vollständige Managed Object auf einem Heartbeat erneut zu senden kostet jedes Mal und speichert keine neue Information. Es nur bei Änderung zu senden streicht die ganze Position. (Ein abgelehnter Schreibvorgang zählt dagegen überhaupt nicht — Fehlschläge sind kostenlos.)',
  ...
  'explain.rule.body': '**Senden Sie jedes Mal dieselben Werte.** Ein Umschlag, dessen Inhalt von Sendung zu Sendung wechselt, verschlechtert später Schreib- und Abfrageleistung.\n\nDeshalb gehört ein Schalter, der sich zweimal pro Stunde ändert, nicht in ein Bündel, das jede Minute abgetastet wird — er würde den Umschlag zwingen, seine Form zu ändern. Er bekommt einen eigenen, gesendet, wenn er sich wirklich ändert.',
